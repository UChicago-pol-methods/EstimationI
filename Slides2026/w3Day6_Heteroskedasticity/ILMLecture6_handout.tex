...
\begin{itemize}
\item \textbf{Classical SEs} dramatically under-cover ($\sim$70--60\%) and \emph{get worse} with heavier tails---more data does not help because the variance formula is wrong, not just imprecise.\pause
\item \textbf{HC0 (White)} improves with $n$ but is noticeably below 95\% at small $n$, especially with heavy-tailed regressors.\pause
\item \textbf{HC2} reaches nominal coverage faster by correcting the finite-sample bias in $\hat{e}_i^2$.\pause
\end{itemize}
\begin{tcolorbox}[colback=blue!5, colframe=blue!50]
\textbf{Takeaway:} Always use heteroskedasticity-robust SEs---but the version matters.  HC2 (or HC3) should be the default, especially with $n < 200$ or heavy-tailed regressors.
\end{tcolorbox}
\end{frame}
