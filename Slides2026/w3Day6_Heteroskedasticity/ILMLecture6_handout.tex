...
\begin{frame}{HC3 $=$ Prediction Errors in the Sandwich (Lecture 5 Connection)}
\small
From Lecture 5: $\mathbb{E}[\hat{e}_i^2|\bm{X}]=(1-h_{ii})\sigma^2$, so $\hat{e}_i^2$ underestimates $\sigma^2$.

\smallskip
\textbf{HC2} divides once by $(1-h_{ii})$:\quad
$\dfrac{\hat{e}_i^2}{1-h_{ii}} \;\Longrightarrow\; \mathbb{E}\!\left[\dfrac{\hat{e}_i^2}{1-h_{ii}}\bigg|\bm{X}\right]=\sigma^2$ \quad (unbiased)\pause

\smallskip
\textbf{HC3} divides twice, using the \textbf{prediction error} $\tilde{e}_i=\hat{e}_i/(1-h_{ii})$:

\vspace{-4pt}
$$\frac{\hat{e}_i^2}{(1-h_{ii})^2}=\tilde{e}_i^2 \quad\Longrightarrow\quad \mathbb{E}\!\left[\tilde{e}_i^2\big|\bm{X}\right]=\frac{\sigma^2}{1-h_{ii}}>\sigma^2$$

\vspace{-6pt}\pause
The second division \emph{deliberately} overestimates $\sigma^2$ by $(1-h_{ii})^{-1}$.  Why?\pause

\vspace{-2pt}
\begin{tcolorbox}[colback=blue!5, colframe=blue!50]
The sandwich meat $\bm{X'DX}$ is estimated with downward finite-sample bias.  HC3 counteracts this by inflating the plug-in variance, especially for high-leverage points where the bias is worst.  MacKinnon \& White (1985) show HC3 $\approx$ the jackknife variance estimator.
\end{tcolorbox}
\end{frame}

\begin{frame}{Many Covariates: A Second Reason to Prefer HC3}
\small
The finite-sample story above treats $k$ as fixed and $n$ large.  But modern empirical work routinely includes \emph{many} covariates---two-way fixed effects, saturated interactions, group dummies---so $k$ is a nontrivial fraction of $n$.\pause

\smallskip
\textbf{Cattaneo, Jansson \& Newey (2018)} study OLS inference under asymptotics where $k/n \not\to 0$:
\begin{itemize}
\item \alert{HC0, HC1, HC2 are all \emph{inconsistent}} in this regime---they systematically under-estimate $\text{Var}(\hat{\beta})$.\pause
\item \textbf{HC3 remains asymptotically valid}, though upward-biased (conservative).\pause
\item The bias grows with the average leverage $\bar{h}=k/n$: when you add fixed effects or rich interactions, every $h_{ii}$ rises, and HC0/HC1/HC2 degrade.
\end{itemize}\pause

\begin{tcolorbox}[colback=blue!5, colframe=blue!50]
So HC3 has two justifications: \textbf{(i)} finite-sample leverage bias (MacKinnon--White), and \textbf{(ii)} robustness to many-covariates asymptotics (Cattaneo--Jansson--Newey).  Both point the same direction---use HC3, especially with many fixed effects or dummies.
\end{tcolorbox}
\end{frame}
...
\item \textbf{Use HC3 by default} for cross-sectional data.  It corrects leverage-induced downward bias (MacKinnon \& White 1985) and, unlike HC0--HC2, stays valid when $k/n$ is non-negligible (Cattaneo, Jansson \& Newey 2018)---the typical case with many fixed effects or interactions. \pause
