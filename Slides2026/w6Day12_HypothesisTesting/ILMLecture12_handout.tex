...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}{Solving Inequality CLS: KKT Conditions}

Lagrangian: $L(\bm{\beta}, \bm{\lambda}) = \tfrac{1}{2}(\bm{Y} - \bm{X}\bm{\beta})'(\bm{Y} - \bm{X}\bm{\beta}) + \bm{\lambda}'(\bm{R}'\bm{\beta} - \bm{r})$.

\pause
\smallskip

\textbf{KKT conditions} at the optimum:
\begin{align*}
\bm{X}'\bm{X}\tilde{\bm{\beta}} - \bm{X}'\bm{Y} + \bm{R}\tilde{\bm{\lambda}} &= \bm{0} \quad &&\text{(stationarity)} \\
\bm{R}'\tilde{\bm{\beta}} &\leq \bm{r} \quad &&\text{(primal feasibility)} \\
\tilde{\bm{\lambda}} &\geq \bm{0} \quad &&\text{(dual feasibility)} \\
\tilde{\lambda}_j\,(\bm{R}_j'\tilde{\bm{\beta}} - r_j) &= 0 \quad &&\text{(complementary slackness)}
\end{align*}
...
\textbf{Two cases:}
\begin{itemize}
\item $\hat{\bm{\beta}}_{\text{OLS}} \in \mathcal{B}$: no constraint binds, so $\tilde{\bm{\beta}} = \hat{\bm{\beta}}_{\text{OLS}}$.
\item $\hat{\bm{\beta}}_{\text{OLS}} \notin \mathcal{B}$: some constraints bind ($\tilde{\lambda}_j > 0$); the binding constraints behave as equalities and we recover the equality-CLS formula on that face.
\end{itemize}

\pause
\smallskip

This is a \emph{quadratic program}.  In R: \texttt{quadprog::solve.QP()}.
\end{frame}
...
Inequality CLS & Feasible set $\mathcal{B}$ as a polyhedron; quadratic program with KKT \\
