...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}{Variance of the CLS Estimator (Robust)}

For SEs and CIs on $\tilde{\bm{\beta}}_{\text{cls}}$ itself, we need its asymptotic variance.  Hansen \S 8.7, no homoskedasticity assumed.

\pause
\smallskip

\textbf{Robust ``meat'' with restricted residuals.}\, Let $\hat{\bm{Q}}_{XX} = \tfrac{1}{n}\bm{X}'\bm{X}$.
$$\tilde{\bm{\Omega}} \;=\; \frac{1}{n-k+q}\sum_{i=1}^n \bm{X}_i\bm{X}_i'\,\tilde{e}_i^2, \qquad \tilde{\bm{V}}_\beta \;=\; \hat{\bm{Q}}_{XX}^{-1}\,\tilde{\bm{\Omega}}\,\hat{\bm{Q}}_{XX}^{-1}.$$
The Lecture~11 sandwich, but with $\tilde{e}_i$ and an $n-k+q$ d.f.\ adjustment.

\pause
\smallskip

\textbf{Project onto the constraint.}\, With $\tilde{\bm{A}} = \hat{\bm{Q}}_{XX}^{-1}\bm{R}\bigl(\bm{R}'\hat{\bm{Q}}_{XX}^{-1}\bm{R}\bigr)^{-1}\bm{R}'$ --- the matrix that maps $\hat{\bm{\beta}}_{\text{ols}}$ to $\tilde{\bm{\beta}}_{\text{cls}}$:
$$\boxed{\;\tilde{\bm{V}}_{\text{cls}} \;=\; (\bm{I} - \tilde{\bm{A}})\,\tilde{\bm{V}}_\beta\,(\bm{I} - \tilde{\bm{A}})'\;}$$
Hansen \S 8.7 unfolds this into four terms.  SE for $\bm{h}'\tilde{\bm{\beta}}_{\text{cls}}$ ($\bm{h}$ outside the row space of $\bm{R}$): $s(\bm{h}'\tilde{\bm{\beta}}_{\text{cls}}) = \sqrt{n^{-1}\bm{h}'\tilde{\bm{V}}_{\text{cls}}\bm{h}}$.
\end{frame}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}{Worked Example: CLS for a Linear Restriction}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]{Worked Example: Two Ways to Compute CLS}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}{Inequality CLS as a Projection}

Conceptually, you are projecting $\hat{\bm{\beta}}_{\text{OLS}}$ onto the feasible region $\mathcal{B}$.

\pause
\bigskip

Picking which constraints bind is what makes the inequality case harder than the equality case --- in practice this is solved by a \emph{quadratic program}.  In R: \texttt{quadprog::solve.QP()}.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}{NSW: Naive ATE and a Missing-Data Thought Experiment}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}{Score Tests: More Settings Where They Help}
...
\begin{itemize}
\item \textbf{Serial correlation} (Breusch--Godfrey): null is OLS with iid errors.
\item \textbf{Omitted nonlinearities}: null is a simple linear specification.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}{Test Inversion vs.\ Delta Method: Hansen's Example}

\textbf{Setup.}  Peak experience in a wage equation, $\theta = -50\beta_1/\beta_2$.

\pause
\bigskip

\begin{itemize}
\item \textbf{Delta method CI:} $[29.8,\; 29.9]$
\item \textbf{Inverted linear-test CI:} $[29.1,\; 30.6]$
\end{itemize}

\pause
\bigskip

The delta method interval is \textbf{far too narrow} --- the ratio's sampling distribution is poorly approximated by a normal centered at $\hat{\theta}$.  Inverting the test for the equivalent linear restriction does not require that approximation.
\end{frame}
