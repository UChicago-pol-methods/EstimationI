...
\begin{itemize}
\item Scalar case: $\text{Var}(aX+bY)=a^2\text{Var}(X)+b^2\text{Var}(Y)+2ab\,\text{Cov}(X,Y)$\pause
\item To generalize, we need a way to collect all variances and covariances in one object.\pause
\item For a random vector $\bm{X}=(X_1,\ldots,X_k)'$, define the \textbf{variance-covariance matrix}:
$$\text{Var}(\bm{X})=\mathbb{E}[(\bm{X}-\mathbb{E}[\bm{X}])(\bm{X}-\mathbb{E}[\bm{X}])']=\bm{\Sigma}$$\pause
\item $\bm{\Sigma}$ is $k\times k$, symmetric, and positive semi-definite.
\item Diagonal entries are variances; off-diagonal entries are covariances.
\end{itemize}
\end{frame}
