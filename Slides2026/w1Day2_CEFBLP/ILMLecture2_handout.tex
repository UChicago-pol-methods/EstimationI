...
# lm() does the same thing
coef(lm(y ~ x1 + x2))  # same answer
\end{lstlisting}
\small
\texttt{solve(A, b)} solves $Ax=b$ via matrix factorization without ever computing $A^{-1}$. This is more numerically stable than \texttt{solve(A) \%*\% b}, which explicitly inverts $A$ and amplifies floating-point errors.
\end{frame}
