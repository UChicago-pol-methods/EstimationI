...
\def\die#1{{\domino#1}}
\usepackage{tikz}
\usetikzlibrary{arrows,fit,positioning,calc,decorations.pathreplacing}
\usepackage{colortbl}
\renewcommand{\familydefault}{cmss}
%\usepackage[all]{xy}
...
\begin{itemize}
\item Define the CEF error: $e\equiv Y-m(X)$, so 
...
\end{align*}
\item $\mathbb{E}[e|X]=0$ is called \emph{mean independence}.
\end{itemize}
\end{frame}
...
\pause
\item If $\operatorname{Cov}(S,N)= 0$, we get a \textbf{clean decomposition}:
...
The diagram on the previous slide is an \emph{analogy}, not a literal picture.  Random variables do not live in $\mathbb{R}^3$---they live in an abstract \textbf{Hilbert space} $L^2$.
...
\begin{itemize}
\item The CEF provides: summarization, MMSE prediction, and decomposition.\pause
...
\begin{frame}{Why Write $\mathbb{E}[Xe]=0$?}
\begin{itemize}
\item Saying ``$X$ is exogenous'' is intuitive but vague.  Writing $\mathbb{E}[Xe]=0$ buys you three things:\pause
\begin{enumerate}
\item \textbf{It makes assumptions precise.}  ``Exogenous'' could mean many things.  $\mathbb{E}[Xe]=0$ says \emph{exactly} what must hold: the prediction error $e = Y - X'\beta$ is uncorrelated with $X$.\pause
\item \textbf{It generalizes.}  Swap $X$ for an instrument $Z$ and you get IV: $\mathbb{E}[Ze]=0$.  Add more conditions and you get GMM.  Every estimator in this course is defined by \emph{which} moment conditions it solves.\pause
\item \textbf{It defines the parameter.}  The population $\beta$ is the value that makes $\mathbb{E}[X(Y - X'\beta)]=0$.  The OLS formula $(\bm{X}'\bm{X})^{-1}\bm{X}'\bm{Y}$ solves the sample analog of this condition.
\end{enumerate}
\end{itemize}
\end{frame}
...
\begin{tcolorbox}[colback=blue!5, colframe=blue!50]
\begin{enumerate}
\item \textbf{Substantive claim:}  A mechanism, institution, or design feature creates a restriction on the joint distribution.
\item \textbf{Moment condition:}  Translate that claim into $\mathbb{E}[g(W, \theta)]=0$.
\item \textbf{Estimator:}  Find $\hat{\theta}$ that solves the sample analog.
\end{enumerate}
\end{tcolorbox}

\smallskip\pause
\textbf{Example:} In a randomized experiment, random assignment means potential outcomes are independent of treatment.  This implies $\mathbb{E}[De]=0$, and OLS consistently estimates the ATE.

\smallskip\pause
\textbf{Counterexample:} If subjects self-select into treatment based on expected benefits, $\mathbb{E}[De]\neq 0$.  OLS is inconsistent; you need a \emph{different} moment condition (e.g., from an instrument).
\textbf{The question is always:} \alert{Why should this moment condition hold?}
\end{frame}


\begin{frame}[fragile]{BLP in R: Bivariate Case}
...
\begin{frame}[fragile]{BLP in R: Multiple Regressors}
Now that we have the matrix formula $\beta = \mathbb{E}[XX']^{-1}\mathbb{E}[XY]$, we can compute it directly:
\begin{lstlisting}[language=R]
# Build the design matrix (include a column of 1s)
X <- cbind(1, x1, x2)

# Solve (X'X) beta = X'y directly, without inverting
beta_matrix <- solve(t(X) %*% X, t(X) %*% y)

# lm() gives the same coefficients, but uses QR
coef(lm(y ~ x1 + x2))
\end{lstlisting}
\small
\texttt{solve(A, b)} solves $Ax=b$ via LU factorization without computing $A^{-1}$, which is more stable than \texttt{solve(A) \%*\% b}. But \texttt{lm()} is even better: it uses QR decomposition on $X$ directly, avoiding the formation of $X'X$ entirely (which squares the condition number).
\end{frame}
...
%% ── Animated partitioned regression sequence ──────────────────────────
\begin{frame}{From Scalar to Partitioned Regression}
\begin{center}
\begin{tikzpicture}[
  every node/.style={inner sep=1pt},
  termbox/.style={rounded corners=3pt, thick, inner sep=4pt},
]
% --- Overlay 1-2: full scalar equation with named nodes for each group ---
\node<1-2| handout:0>[anchor=base west] (lhs) at (0,0) {$Y ={}$};
\node<1-2| handout:0>[anchor=base west] (grpA) at (lhs.base east) {$\beta_0 + \beta_1 x_1 + \beta_2 x_2$};
\node<1-2| handout:0>[anchor=base west] (plus1) at (grpA.base east) {${}+{}$};
\node<1-2| handout:0>[anchor=base west] (grpB) at (plus1.base east) {$\beta_3 x_3 + \beta_4 x_4 + \beta_5 x_5$};
\node<1-2| handout:0>[anchor=base west] (err) at (grpB.base east) {${}+ e$};

% --- Overlay 2: draw grouping boxes using fit ---
\only<2| handout:0>{%
  \node[termbox, draw=blue, fit=(grpA)] {};
  \node[blue, font=\footnotesize, above=2pt of grpA] {$\bm{X}_1$};
  \node[termbox, draw=darkgreen, fit=(grpB)] {};
  \node[darkgreen, font=\footnotesize, above=2pt of grpB] {$\bm{X}_2$};
}

% --- Overlay 3: collapsed vector form ---
\only<3| handout:0>{%
  \node[anchor=base west] at (0,0) {%
    $Y = \textcolor{blue}{\bm{X}_1'\bm{\beta}_1}
        + \textcolor{darkgreen}{\bm{X}_2'\bm{\beta}_2} + e,
        \qquad \mathbb{E}[\bm{X}e]=\bm{0}$};
}

% --- Overlay 4: show what X looks like ---
\only<4-| handout:1>{%
  \node[anchor=base west] at (0,0.8) {%
    $Y = \textcolor{blue}{\bm{X}_1'\bm{\beta}_1}
        + \textcolor{darkgreen}{\bm{X}_2'\bm{\beta}_2} + e$};
  \node[anchor=west] at (0,-0.3) {%
    $\bm{X}_1 = \textcolor{blue}{\begin{pmatrix} 1\\[2pt] x_1\\[2pt] x_2\end{pmatrix}}, \qquad
    \bm{X}_2 = \textcolor{darkgreen}{\begin{pmatrix} x_3\\[2pt] x_4\\[2pt] x_5\end{pmatrix}}, \qquad
    \bm{X} = \begin{pmatrix} \textcolor{blue}{\bm{X}_1}\\[2pt]
    \textcolor{darkgreen}{\bm{X}_2}\end{pmatrix}$};
}
\end{tikzpicture}
\end{center}
\end{frame}
...
\begin{frame}{Building the Moment Matrices}
\begin{center}
\begin{tikzpicture}
% --- Overlay 1: Q_XX in expectations ---
\only<1| handout:0>{%
  \node at (0,0) {$\mathbf{Q}_{XX} = \mathbb{E}[\bm{X}\bm{X}']
    = \mathbb{E}\!\left[\begin{pmatrix} \bm{X}_1\\[2pt] \bm{X}_2\end{pmatrix}
    \begin{pmatrix} \bm{X}_1' & \bm{X}_2'\end{pmatrix}\right]
    = \begin{bmatrix}
    \mathbb{E}[\bm{X}_1\bm{X}_1'] & \mathbb{E}[\bm{X}_1\bm{X}_2']\\[4pt]
    \mathbb{E}[\bm{X}_2\bm{X}_1'] & \mathbb{E}[\bm{X}_2\bm{X}_2']
    \end{bmatrix}$};
}
% --- Overlay 2: replace with Q notation ---
\only<2| handout:0>{%
  \node at (0,0) {$\mathbf{Q}_{XX}
    = \begin{bmatrix}
    \mathbb{E}[\bm{X}_1\bm{X}_1'] & \mathbb{E}[\bm{X}_1\bm{X}_2']\\[4pt]
    \mathbb{E}[\bm{X}_2\bm{X}_1'] & \mathbb{E}[\bm{X}_2\bm{X}_2']
    \end{bmatrix}
    \;\equiv\;
    \begin{bmatrix}
    \textcolor{blue}{\mathbf{Q}_{11}} & \mathbf{Q}_{12}\\[4pt]
    \mathbf{Q}_{21} & \textcolor{darkgreen}{\mathbf{Q}_{22}}
    \end{bmatrix}$};
}
% --- Overlay 3: compact form + Q_XY ---
\only<3-| handout:1>{%
  \node at (0,0.5) {$\mathbf{Q}_{XX}
    = \begin{bmatrix}
    \textcolor{blue}{\mathbf{Q}_{11}} & \mathbf{Q}_{12}\\[4pt]
    \mathbf{Q}_{21} & \textcolor{darkgreen}{\mathbf{Q}_{22}}
    \end{bmatrix}$};
  \node at (0,-1.0) {$\mathbf{Q}_{XY}
    = \mathbb{E}[\bm{X}Y]
    = \begin{bmatrix}
    \mathbb{E}[\bm{X}_1 Y]\\[4pt] \mathbb{E}[\bm{X}_2 Y]
    \end{bmatrix}
    \;\equiv\;
    \begin{bmatrix}
    \textcolor{blue}{\mathbf{Q}_{1Y}}\\[4pt]
    \textcolor{darkgreen}{\mathbf{Q}_{2Y}}
    \end{bmatrix}$};
}
\end{tikzpicture}
\end{center}
...
\begin{frame}{The Dot Notation: Variation ``Left Over''}
\onslide<1->{%
Recall the projection error variance: $\;\sigma^2_e = Q_{YY} - \bm{Q}_{YX}\bm{Q}_{XX}^{-1}\bm{Q}_{XY} \;\equiv\; Q_{YY\cdot X}$\\[8pt]}
\onslide<2->{%
The same pattern works for \emph{any} partition block:}
\begin{center}
\begin{tikzpicture}
\only<3| handout:0>{%
  \node at (0,0) {$\mathbf{Q}_{11\cdot 2}
    \;\equiv\; \underbrace{\mathbf{Q}_{11}}_{\text{total var of }\bm{X}_1}
    \;-\; \underbrace{\mathbf{Q}_{12}\,\mathbf{Q}_{22}^{-1}\,\mathbf{Q}_{21}}_%
    {\text{part predicted by }\bm{X}_2}$};
}
\only<4-| handout:1>{%
  \node at (0,0.3) {$\mathbf{Q}_{11\cdot 2}
    \;\equiv\; \underbrace{\mathbf{Q}_{11}}_{\text{total}}
    \;-\; \underbrace{\mathbf{Q}_{12}\,\mathbf{Q}_{22}^{-1}\,\mathbf{Q}_{21}}_%
    {\text{predicted by }\bm{X}_2}$};
  \node at (0,-1.1) {$\mathbf{Q}_{1Y\cdot 2}
    \;\equiv\; \underbrace{\mathbf{Q}_{1Y}}_{\text{total}}
    \;-\; \underbrace{\mathbf{Q}_{12}\,\mathbf{Q}_{22}^{-1}\,\mathbf{Q}_{2Y}}_%
    {\text{predicted by }\bm{X}_2}$};
}
\end{tikzpicture}
\end{center}
...
$\mathbf{Q}_{a\cdot b}$ = the variation in $a$ that $b$ \textbf{cannot predict}.
\end{tcolorbox}}
\end{frame}


\begin{frame}{Two Equations, Two Unknowns}
The normal equation $\mathbf{Q}_{XY}=\mathbf{Q}_{XX}\bm{\beta}$ expands to:
\begin{center}
\begin{tikzpicture}
% --- Overlay 1: matrix form ---
\only<1| handout:0>{%
  \node at (0,0) {$\begin{bmatrix} \mathbf{Q}_{1Y}\\[4pt] \mathbf{Q}_{2Y}\end{bmatrix}
    = \begin{bmatrix} \mathbf{Q}_{11} & \mathbf{Q}_{12}\\[4pt]
    \mathbf{Q}_{21} & \mathbf{Q}_{22}\end{bmatrix}
    \begin{bmatrix} \bm{\beta}_1\\[4pt] \bm{\beta}_2\end{bmatrix}$};
}
% --- Overlay 2: two separate equations ---
\only<2-| handout:1>{%
  \node[anchor=west] (r1) at (-4,0.5) {%
    $\mathbf{Q}_{1Y}=\mathbf{Q}_{11}\bm{\beta}_1+\mathbf{Q}_{12}\bm{\beta}_2$};
  \node[anchor=west, right=4pt of r1, blue, font=\footnotesize] {(Row 1)};
  \node[anchor=west] (r2) at (-4,-0.5) {%
    $\mathbf{Q}_{2Y}=\mathbf{Q}_{21}\bm{\beta}_1+\mathbf{Q}_{22}\bm{\beta}_2$};
  \node[anchor=west, right=4pt of r2, darkgreen, font=\footnotesize] {(Row 2)};
}
% --- Overlay 3: highlight it's substitution ---
\only<3->{%
  \node[anchor=west, font=\small] at (-4,-1.5)
    {This is just two simultaneous equations---solve by \textbf{substitution}.};
}
\end{tikzpicture}
\end{center}
\end{frame}
...
\begin{frame}{Eliminate $\bm{\beta}_2$}
\textbf{Step 1.}\; Solve \textcolor{darkgreen}{Row 2} for $\bm{\beta}_2$:\par
\onslide<1->{%
$$\bm{\beta}_2=\mathbf{Q}_{22}^{-1}\bigl(\mathbf{Q}_{2Y}
  -\mathbf{Q}_{21}\bm{\beta}_1\bigr)$$}
\onslide<2->{%
\textbf{Step 2.}\; Substitute into \textcolor{blue}{Row 1}:
$$\mathbf{Q}_{1Y}=\mathbf{Q}_{11}\bm{\beta}_1
  +\mathbf{Q}_{12}\,\textcolor{darkgreen}{\mathbf{Q}_{22}^{-1}
  \bigl(\mathbf{Q}_{2Y}-\mathbf{Q}_{21}\bm{\beta}_1\bigr)}$$}
\onslide<3->{%
\textbf{Step 3.}\; Collect $\bm{\beta}_1$ terms and move the rest left:
$$\underbrace{\mathbf{Q}_{1Y}-\mathbf{Q}_{12}\mathbf{Q}_{22}^{-1}\mathbf{Q}_{2Y}}_%
  {\textcolor{blue}{\mathbf{Q}_{1Y\cdot 2}}}
  =\underbrace{\bigl(\mathbf{Q}_{11}
  -\mathbf{Q}_{12}\mathbf{Q}_{22}^{-1}\mathbf{Q}_{21}\bigr)}_%
  {\textcolor{blue}{\mathbf{Q}_{11\cdot 2}}}\;\bm{\beta}_1$$}
\end{frame}
...
\begin{center}
\begin{tikzpicture}
% --- Overlay 1: full expression ---
\only<1| handout:0>{%
  \node at (0,1) {From the previous slide:};
  \node at (0,0) {$\bigl(\mathbf{Q}_{11}
    -\mathbf{Q}_{12}\mathbf{Q}_{22}^{-1}\mathbf{Q}_{21}\bigr)\bm{\beta}_1
    = \mathbf{Q}_{1Y}-\mathbf{Q}_{12}\mathbf{Q}_{22}^{-1}\mathbf{Q}_{2Y}$};
}
% --- Overlay 2: replace with dot notation ---
\only<2| handout:0>{%
  \node at (0,1) {Substitute the dot notation:};
  \node at (0,0) {$\mathbf{Q}_{11\cdot 2}\;\bm{\beta}_1
    = \mathbf{Q}_{1Y\cdot 2}$};
}
% --- Overlay 3: solve ---
\only<3-| handout:1>{%
  \node at (0,1) {Premultiply by $\mathbf{Q}_{11\cdot 2}^{-1}$:};
  \node at (0,0) {$\boxed{\bm{\beta}_1
    = \mathbf{Q}_{11\cdot 2}^{-1}\;\mathbf{Q}_{1Y\cdot 2}}$};
  \node[anchor=north, text width=10cm, align=center, font=\small] at (0,-0.8)
    {By symmetry:\;
    $\bm{\beta}_2=\mathbf{Q}_{22\cdot 1}^{-1}\;\mathbf{Q}_{2Y\cdot 1}$};
}
% --- Overlay 4: interpretation ---
\only<4-| handout:1>{%
  \node[anchor=north] at (0,-1.6) {%
    \begin{tcolorbox}[colback=blue!5, colframe=blue!50!black, width=11cm]
    $\bm{\beta}_1$ depends only on the variation in $\bm{X}_1$ and $Y$
    that $\bm{X}_2$ \textbf{cannot explain}.
    \end{tcolorbox}};
}
\end{tikzpicture}
\end{center}
...
\begin{frame}{From Block Elimination to Block Inverse}
The block elimination gave us $\bm{\beta}=\bm{Q}_{XX}^{-1}\bm{Q}_{XY}$.\; What does $\bm{Q}_{XX}^{-1}$ itself look like?\\
\medskip
\onslide<1->{%
We want $\begin{bmatrix} \mathbf{M}_{11} & \mathbf{M}_{12}\\ \mathbf{M}_{21} & \mathbf{M}_{22}\end{bmatrix}$ such that
$\begin{bmatrix} \mathbf{Q}_{11} & \mathbf{Q}_{12}\\ \mathbf{Q}_{21} & \mathbf{Q}_{22}\end{bmatrix}\begin{bmatrix} \mathbf{M}_{11} & \mathbf{M}_{12}\\ \mathbf{M}_{21} & \mathbf{M}_{22}\end{bmatrix}=\begin{bmatrix} \mathbf{I} & \mathbf{0}\\ \mathbf{0} & \mathbf{I}\end{bmatrix}$}\\[6pt]
\onslide<2->{%
\textbf{Same elimination trick.}\; Focus on the first column:
\begin{align}
\mathbf{Q}_{11}\mathbf{M}_{11}+\mathbf{Q}_{12}\mathbf{M}_{21}&=\mathbf{I} \tag{A}\\
\mathbf{Q}_{21}\mathbf{M}_{11}+\mathbf{Q}_{22}\mathbf{M}_{21}&=\mathbf{0} \tag{B}
\end{align}}
\onslide<3->{%
Solve (B) for $\mathbf{M}_{21}$, substitute into (A)\;---\;exactly the same steps as before:
$$\mathbf{Q}_{11\cdot 2}\;\mathbf{M}_{11}=\mathbf{I}
  \qquad\Longrightarrow\qquad
  \boxed{\mathbf{M}_{11}=\mathbf{Q}_{11\cdot 2}^{-1}}$$}
\end{frame}


\begin{frame}{The Complete Block Inverse}
\onslide<1->{%
Collecting all four blocks:
$$\bm{Q}_{XX}^{-1}=\begin{bmatrix}
  \textcolor{blue}{\mathbf{Q}_{11\cdot 2}^{-1}}
  & -\mathbf{Q}_{11\cdot 2}^{-1}\mathbf{Q}_{12}\mathbf{Q}_{22}^{-1}\\[6pt]
  -\mathbf{Q}_{22}^{-1}\mathbf{Q}_{21}\mathbf{Q}_{11\cdot 2}^{-1}
  & \textcolor{darkgreen}{\mathbf{Q}_{22\cdot 1}^{-1}}
\end{bmatrix}$$}
\onslide<2->{%
Multiply $\bm{Q}_{XX}^{-1}\bm{Q}_{XY}$ to recover $\bm{\beta}_1$:}
\begin{center}
\begin{tikzpicture}
\only<2| handout:0>{%
  \node at (0,0) {$\bm{\beta}_1
    = \mathbf{Q}_{11\cdot 2}^{-1}\mathbf{Q}_{1Y}
    -\mathbf{Q}_{11\cdot 2}^{-1}\mathbf{Q}_{12}\mathbf{Q}_{22}^{-1}\mathbf{Q}_{2Y}$};
}
\only<3| handout:0>{%
  \node at (0,0) {$\bm{\beta}_1
    = \mathbf{Q}_{11\cdot 2}^{-1}
    \bigl(\mathbf{Q}_{1Y}
    -\mathbf{Q}_{12}\mathbf{Q}_{22}^{-1}\mathbf{Q}_{2Y}\bigr)$};
}
\only<4-| handout:1>{%
  \node at (0,0) {$\bm{\beta}_1
    = \mathbf{Q}_{11\cdot 2}^{-1}\;\mathbf{Q}_{1Y\cdot 2}
