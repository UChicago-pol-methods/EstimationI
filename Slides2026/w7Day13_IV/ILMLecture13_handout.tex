...
\begin{itemize}
\item The \textbf{reduced form} is the linear projection of each endogenous variable on the instruments $\bm{z}$:
$$\bm{y}_2 = \Gamma'\bm{z}+\bm{u}_2, \qquad \Gamma = \mathbb{E}[\bm{z}\bm{z}']^{-1}\mathbb{E}[\bm{z}\bm{y}_2'], \qquad \mathbb{E}[\bm{z}\bm{u}_2']=0$$
\item Because $\bm{z}$ is exogenous, OLS \textbf{consistently estimates} $\Gamma$ --- this is what makes IV work.
\item Partition $\Gamma$ to match $\bm{z}=(\bm{z}_1', \bm{z}_2')'$:
$$\bm{y}_2 = \Gamma_{12}'\bm{z}_1+\Gamma_{22}'\bm{z}_2+\bm{u}_2$$
...
\begin{itemize}
\item For just-identified IV ($l=k$), $\hat{\beta}_{IV}=(Z'X)^{-1}Z'Y$, and
$$\sqrt{n}(\hat{\beta}_{IV}-\beta) = \left(\frac{Z'X}{n}\right)^{-1}\frac{Z'e}{\sqrt{n}}.$$
\item WLLN: $\dfrac{Z'X}{n}\xrightarrow{p}Q\equiv\mathbb{E}[\bm{z}\bm{x}']$ ($k\times k$ since $l=k$).
...
\end{tcolorbox}
The general 2SLS sandwich (next slide) reduces to this when $l=k$.
\end{frame}
