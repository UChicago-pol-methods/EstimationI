...
\begin{frame}{Example: The Distribution of a Sum (Two Fair Dice)}
...
\]
The CLT proof exploits this at every step.
...
\emph{Odd moments vanish (symmetry); even moments are $1, 3, 15, 105, \ldots$. One function $e^{t^2/2}$ encodes all of them.}
...
\textbf{Compare the density domain.} Adding independent RVs convolves densities, $f_{X+Y}(z) = \int f_X(x)\,f_Y(z-x)\,dx$ --- a painful integral. In the MGF domain, a product.
...
\textbf{Why we need this.} Convergence of CDFs is hard to show directly; MGFs are analytic. The theorem lets us work at the level of MGFs and \emph{conclude} convergence in distribution for free.
...
\[
\bm{C}(\bm{b}) \;=\; \frac{\partial \bm{f}(\bm{b})}{\partial \bm{b}'}.
\]
...
\textbf{Evaluate at the truth.} Consistency of $\bm{b}$ plus continuity of $\bm{C}$:
\[
\operatorname{plim}\bm{C}(\bm{b}) \;=\; \frac{\partial \bm{f}(\bm{\beta})}{\partial \bm{\beta}'} \;\equiv\; \bm{\Gamma}. \qquad\text{(Slutsky)}
\]
...
\textbf{First-order Taylor expansion} from $\bm{\beta}$ to $\bm{b}$:
\[
\bm{f}(\bm{b}) \;=\; \bm{f}(\bm{\beta}) \;+\; \bm{\Gamma}\,(\bm{b} - \bm{\beta}) \;+\; O_p(n^{-1}).
\]
...
Combine the Taylor expansion with $\bm{b} \overset{a}{\sim} N(\bm{\beta},\ \operatorname{Asy.Var}(\bm{b}))$:
\[
\bm{f}(\bm{b}) \;\overset{a}{\sim}\; N\!\Bigl(\bm{f}(\bm{\beta}),\; \bm{\Gamma}\,[\operatorname{Asy.Var}(\bm{b})]\,\bm{\Gamma}'\Bigr).
\]
...
This is nonlinear --- $\bm{R}$ itself depends on $\beta$, estimated at $\hat\beta$.
