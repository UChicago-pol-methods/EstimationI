...
$$\bm{b}\overset{a}{\sim} N\!\left[\bm{\beta},\ \frac{\sigma^2}{n}  \left[\text{plim}\frac{\bm{X'X}}{n}\right]^{-1}\right], \qquad \text{estimated by } s^2(\bm{X}'\bm{X})^{-1}$$
...
\item Estimate this approximation with $\bm{C}[s^2(\bm{X}'\bm{X})^{-1}]\bm{C}'$. \textit{(HW4 Q1b: $g(x)=e^x \Rightarrow \sigma^2 e^{2\mu}$.)}
\end{itemize}
\end{frame}
...
\begin{frame}{Delta Method for the Long-Run Elasticities}

Stack the two long-run elasticities as $\bm{f}(\bm{\beta}) = \bigl(\beta_2/(1-\beta_4),\ \beta_3/(1-\beta_4)\bigr)'$. The gradient is
\[
\bm{C}(\bm{\beta}) = \frac{\partial \bm{f}}{\partial \bm{\beta}'} =
\begin{bmatrix}
0 & \tfrac{1}{1-\beta_4} & 0 & \tfrac{\beta_2}{(1-\beta_4)^2} \\
...
Plug $\hat{\bm{\beta}}$ into $\bm{C}$ and sandwich:
\[
\widehat{\Var}(\hat{\bm{f}}) = \bm{C}(\hat{\bm{\beta}})\,\hat{\bm{V}}_{\hat\beta}\,\bm{C}(\hat{\bm{\beta}})'.
\]
The rows of $\bm{C}$ are gradient vectors --- one per scalar function we want a SE for.
...
The remainder vanishes: $\hat{e}_i - e_i = X_i'(\beta - \hat\beta) \overset{p}{\to} 0$ by consistency of $\hat\beta$.
...
= \frac{\sqrt n(\hat\theta - \theta)}{\sqrt{\hat V_\theta}}
\xrightarrow{d} N(0,1).
\]
...
For the CEF $m(x) = x'\beta$, the target parameter $\theta = r(\beta) = x'\beta$ has gradient $\bm R = x$, so the delta method gives
\[
s(\hat\theta) = \sqrt{\bm x' \bm{\hat V}_{\hat\beta}\, \bm x}.
\]
...
&= \text{plim} \left( \frac{1}{n} \sum_{i=1}^n X_i X_i' + \frac{\lambda}{n} \bm{I}_k \right)^{-1}
\left( \frac{1}{n} \sum_{i=1}^n X_i Y_i \right) \\[4pt]
&= \left( \mathbb{E}[X_i X_i'] + \underbrace{\text{plim}\,\frac{\lambda}{n}}_{\to\, 0}\, \bm{I}_k \right)^{-1}
\mathbb{E}[X_i Y_i] \\[4pt]
&= \left( \mathbb{E}[X_i X_i'] \right)^{-1} \mathbb{E}[X_i Y_i] = \beta
\end{align*}
...
\begin{frame}{Influence Functions and AIPW (Preview)}

The toolkit (CLT, Slutsky, $o_p$ algebra) extends beyond OLS. Many estimators admit an \textbf{influence function representation}:
\[
\sqrt{n}(\hat\theta - \theta) \;=\; \frac{1}{\sqrt{n}}\sum_{i=1}^n \psi(Z_i) \;+\; o_p(1), \quad \mathbb{E}[\psi(Z)]=0,
\]
so the CLT delivers $\sqrt{n}(\hat\theta-\theta)\overset{d}{\to} N(0,\mathrm{Var}(\psi))$. (OLS: $\psi(Z) = Q_{XX}^{-1} X e$.)

\pause

\textbf{AIPW (ATE).} The influence function is
\[
\psi(Z) = \mu_1(X)-\mu_0(X) - \tau + \tfrac{D}{p}(Y-\mu_1(X)) - \tfrac{1-D}{1-p}(Y-\mu_0(X)).
\]

\textit{HW4 Q4--Q9 builds this end to end: Q4 derives $\psi$, Q5 isolates the plug-in remainder $R_n$, Q6 proves Neyman orthogonality $\mathbb{E}[1-D/p\mid X]=0$, Q7 uses conditional Chebyshev to show $R_n = o_p(n^{-1/2})$, Q8 closes with Slutsky.}
