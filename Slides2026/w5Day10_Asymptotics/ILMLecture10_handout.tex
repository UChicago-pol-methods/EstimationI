...
\textbf{The punchline:} When we write $f(x) = g(x) + O(x^3)$, the leftover terms are \emph{at most} on the order of $x^3$. When $x$ is small (like $t/\sqrt{n}$), these terms are negligible.
...
\textbf{Step 3.} Taylor-expand $M_W(s)$ around $s = 0$, using $M_W^{(k)}(0) = \mathbb{E}[W^k]$:
$$M_W(s) = \underbrace{1}_{M_W(0)} + \underbrace{0}_{E[W]}\cdot s + \frac{1}{2}\,s^2 + O(s^3) = 1 + \frac{s^2}{2} + O(s^3)$$

\pause

\textbf{Step 4.} Substitute $s = t/\sqrt{n}$ and raise to the $n$-th power (by independence):
\begin{align*}
M_W\!\left(\frac{t}{\sqrt{n}}\right) &= 1 + \frac{t^2}{2n} + O(n^{-3/2}) \\
M_{Z_n}(t) &= \left[M_W\!\left(\frac{t}{\sqrt{n}}\right)\right]^n = \left[1 + \frac{t^2}{2n} + O(n^{-3/2})\right]^n
\end{align*}
...
\textbf{Step 5.} Apply the limit $(1 + x/n)^n \to e^x$:
$$M_{Z_n}(t) = \left[1 + \frac{t^2/2}{n} + O(n^{-3/2})\right]^n \longrightarrow e^{t^2/2}$$

\pause

The $O(n^{-3/2})$ remainder vanishes faster than $1/n$, so it does not affect the limit. The limit $e^{t^2/2}$ is the MGF of $N(0,1)$.

\pause

\textbf{Step 6.} By the continuity theorem for MGFs:
$$Z_n = \frac{\sqrt{n}(\bar{X}_n - \mu)}{\sigma} \overset{d}{\to} N(0,1) \qquad \blacksquare$$

\pause

\begin{tcolorbox}[colback=blue!5, colframe=blue!50]
Taylor expansion $\to$ $1 + t^2/(2n) + \text{small}$; \;exponential limit $\to$ $e^{t^2/2}$; \;MGF continuity theorem $\to$ convergence in distribution.
\end{tcolorbox}
