...
In published work, $\ell > k$ is the rule, not the exception.  Each row below has more instruments than endogenous regressors, so no $\hat\beta$ can satisfy every sample moment exactly.
...
\smallskip
The first row is genuinely experimental: each cohort's draft-eligibility ceiling gives a separate random instrument for veteran status.  The other rows rest on excludability arguments.  GMM combines $\ell > k$ moments and tests whether they agree (Lecture 16).
