...
\begin{frame}{From 2SLS to Control Function}
\textbf{Recap.}  2SLS replaces $\bm{x}_2$ with its projection on the instruments, $\hat{\bm{x}}_2=P_Z\bm{x}_2$.  Splitting $\bm{x}_2$ accordingly,
$$\bm{x}_2=\hat{\bm{x}}_2+\hat{u}_2,$$
the endogeneity lives entirely in the first-stage residual $\hat{u}_2$, since $\hat{\bm{x}}_2$ is a function of the exogenous $\bm{z}$.

\medskip
\textbf{Two ways to neutralize $\hat{u}_2$:}
\begin{itemize}
\item \alert{2SLS:} \emph{drop} $\hat{u}_2$ and regress $Y$ on $\hat{\bm{x}}_2$.
\item \alert{Control function:} \emph{add} $\hat{u}_2$ as a regressor.  Holding $\hat{u}_2$ fixed, the only remaining variation in $\bm{x}_2$ is the exogenous $\hat{\bm{x}}_2$.
\end{itemize}
\end{frame}


\begin{frame}{Control Function: Regression and Recipe}
\textbf{Linear-projection assumption.}  Let $\alpha=(E[u_2u_2'])^{-1}E[u_2e]$ so that
$$e=u_2'\alpha+v,\qquad E[u_2v]=0.$$
Substituting into the structural equation $Y=\bm{x}_1'\beta_1+\bm{x}_2'\beta_2+e$ gives a regression in which every regressor is orthogonal to the new error:
$$Y=\bm{x}_1'\beta_1+\bm{x}_2'\beta_2+u_2'\alpha+v.$$

\medskip
\textbf{Two-step recipe.}
\begin{enumerate}
\item OLS first stage; save residuals $\hat{u}_2$.
\item OLS of $Y$ on $\bm{x}_1$, $\bm{x}_2$, and $\hat{u}_2$.
\end{enumerate}
Under homoskedasticity, the resulting $\hat{\beta}_2$ coincides numerically with 2SLS.
\end{frame}


\begin{frame}{Hausman Endogeneity Test}
\textbf{Maintained assumption: $\bm{z}$ is a valid instrument.}  Under that assumption the test asks whether IV is \emph{necessary}, not whether the IV is correct.
\begin{itemize}
\item $H_0$: $\bm{x}_2$ exogenous.  Both OLS and IV are consistent; OLS is efficient.
\item $H_1$: $\bm{x}_2$ endogenous.  Only IV is consistent.
\end{itemize}
A large gap between $\hat{\beta}_{OLS}$ and $\hat{\beta}_{IV}$ is evidence against $H_0$:
$$H=(\hat{\beta}_{IV}-\hat{\beta}_{OLS})'\bigl[\text{Avar}(\hat{\beta}_{IV})-\text{Avar}(\hat{\beta}_{OLS})\bigr]^{-1}(\hat{\beta}_{IV}-\hat{\beta}_{OLS})\xrightarrow{d}\chi^2_{k_2}.$$

\medskip
\textbf{Control-function shortcut.}  In the CF regression, $\bm{x}_2$ is exogenous iff $\alpha=0$.  The OLS $t$-test on $\hat{\alpha}$ is asymptotically equivalent to $H$, so endogeneity is read off a single coefficient.

\medskip
\alert{If validity is in doubt, the test conflates the two: rejection has more than one source.}
\end{frame}
...
\begin{lstlisting}
# Step 1: First stage
fs <- lm(X2 ~ Z1 + Z2, data = dat)
dat$u2_hat <- residuals(fs)
# Step 2: Augmented regression
cf <- lm(Y ~ X1 + X2 + u2_hat, data = dat)
summary(cf)
# t on u2_hat: Hausman test (valid)
# coef on X2:  IV point estimate; SE is WRONG
# For correct inference on beta_2, use 2SLS:
estimatr::iv_robust(Y ~ X1+X2 | X1+Z1+Z2, data = dat)
\end{lstlisting}
\small The second-stage OLS SE on $\bm{x}_2$ ignores estimation of $\hat{u}_2$ and uses $\hat{\sigma}_v^2$ in place of $\hat{\sigma}_e^2$.
\end{frame}


\begin{frame}[fragile]{Bootstrapping Both Stages}
\textbf{Pairs bootstrap.}  Resample $(Y_i, \bm{x}_{1i}, \bm{x}_{2i}, \bm{z}_i)$ with replacement, redo \emph{both} stages each draw.  Recomputing $\hat{u}_2$ on every replicate is what propagates first-stage uncertainty.
\begin{lstlisting}
B <- 1000; beta2_boot <- numeric(B)
for (b in seq_len(B)) {
  d <- dat[sample(nrow(dat), replace = TRUE), ]
  d$u2_hat <- residuals(lm(X2 ~ Z1 + Z2, data = d))
  beta2_boot[b] <- coef(lm(Y ~ X1+X2+u2_hat, data = d))["X2"]
}
quantile(beta2_boot, c(0.025, 0.975))  # percentile 95% CI
\end{lstlisting}
\small \textbf{Report the percentile CI, not a bootstrap SE} (Hansen).  Just-identified 2SLS has \emph{infinite} finite-sample variance (next section), so a bootstrap SE targets a quantity that does not exist.  The percentile CI is built from empirical quantiles and remains well defined.
\end{frame}
...
\begin{itemize}
\item A striking result (Kinal 1980, Kiviet): the IV estimator has at most $q=\ell-k$ moments, where $q$ is the number of overidentifying restrictions.
...
\item Consequences:
\begin{itemize}
\item Do \emph{not} report bootstrap standard errors: they target a variance that does not exist (Hansen).  Percentile CIs, built from empirical quantiles, are still well defined.
...
\begin{itemize}
\item When $\ell>k$, we have $q=\ell-k$ ``extra'' moment conditions.
...
\begin{frame}{When Is 2SLS \emph{Actually} LATE? (Blandhol et al.\ 2025)}
\begin{itemize}
\item The LATE interpretation of 2SLS hides a requirement: \alert{rich covariates},
...
\item Otherwise the 2SLS estimand mixes complier effects with always-taker effects, and \emph{some always-taker weights are necessarily negative} --- it is not even a non-negatively-weighted average of subgroup treatment effects.
\item In a survey of 99 published IV-with-covariates papers, BBMT find \emph{one} that used a saturated specification.
\end{itemize}
\end{frame}


\begin{frame}{2SLS-LATE: Practical Workflow (BBMT 2025)}
\begin{enumerate}\itemsep=4pt
\item \textbf{Report 2SLS without covariates} if exogeneity doesn't require them.  Avoid kitchen-sink controls --- adding covariates makes rich covariates \emph{less} likely to hold.
...
\item \textbf{If RESET rejects, estimate $\beta_{\text{rich}}$ via DDML PLIV} and report alongside 2SLS.  R packages: \texttt{ddml}, \texttt{DoubleML}.
...
\item IV has \alert{no finite moments} when just identified; report percentile CIs, not bootstrap SEs.
