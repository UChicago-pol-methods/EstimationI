...
### Application: U.S. House election margins
...
pairs <- house |>
  ...
  filter(year_tp2 == year_t + 2)
```

The 2008→2010 fit matches the midterm's landslide story:
...
c(slope = coef(fit_08)[2],
  ...
  sd_tp2 = sd(pair_08$margin_tp2))
```

Slope `r round(coef(fit_08)[2], 2)`, correlation `r round(cor(pair_08$margin_t, pair_08$margin_tp2), 2)`. Both less than 1 in absolute value, as they must be. The standard deviation is essentially the same across the two cycles --- the compression of individual predictions does not carry over to the marginal distribution.
...
pair_08 |>
  ...
  summarise(n = n(),
            ...
            mean_2010 = mean(margin_tp2))
```

Every group's 2010 mean is closer to the 2010 overall mean than its 2008 mean was to the 2008 overall mean --- the mechanical consequence of $|\beta| < 1$. The analyst's error is reading this as a substantive trend rather than a projection identity.

### The same phenomenon across 24 cycles
...
ggplot(pairs, aes(margin_t, margin_tp2)) +
  ...
  labs(x = "District margin, year t (pp)",
       ...
       subtitle = "Blue: OLS. Dashed: y = x. Slope below 1 compresses extremes toward the mean.")
```
...
A district $50$ points above the mean in one cycle is predicted about `r round(50 * coef(fit_pooled)[2], 1)` points above in the next --- not because districts are converging, but because a projection slope below 1 gives shrunken predictions even when the underlying distribution is stationary.
